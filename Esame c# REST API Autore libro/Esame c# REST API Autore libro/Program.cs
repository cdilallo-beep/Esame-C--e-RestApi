...
var scritture = new List<Scrivere>();

app.MapPost("/api/scrivere", (Scrivere nuovaScrittura) =>
{
    var autore = autori.FirstOrDefault(a => a.AutoreId == nuovaScrittura.AutoreId);
    var libro = libri.FirstOrDefault(l => l.LibroId == nuovaScrittura.LibroId);

    if (autore is null || libro is null)
        return Results.BadRequest("Autore o libro non valido.");

    nuovaScrittura.DataScrittura = DateTime.Now.ToString("yyyy-MM-dd");
    scritture.Add(nuovaScrittura);
    return Results.Created("/api/scritture", nuovaScrittura);
})
.WithName("CreateScrittura")
.WithOpenApi(op => new(op)
{
    Summary = "Registra una scrittura",
    Description = "Associa un autore a un libro e registra la data di scrittura."
})
.Accepts<Scrivere>("application/json")
.Produces<Scrivere>(StatusCodes.Status201Created)
.Produces(StatusCodes.Status400BadRequest);
